...
El CSV compartido es incremental, así que sus denominadores dependen de qué parte se mire. Hay 50 filas del subconjunto histórico (`ronda = 0`) y 404 apariciones revisadas en ese subconjunto histórico, repartidas entre los alcances `herramientas` y `estudios`; sumadas las rondas 1 a 3 de la búsqueda ampliada de estudios, el registro llega a 86 filas en total y 2.036 apariciones en todo el registro. Este estado del arte deriva sus conteos únicamente del subconjunto `alcance = herramientas`, contenido íntegramente en la ronda `0`: 32 búsquedas o inspecciones de fuente y 121 apariciones de herramientas revisadas. Estas apariciones no son 121 registros únicos: incluyen consultas amplias, búsquedas sin resultados y comprobaciones dirigidas de fichas, repositorios, archivos y código fuente. Las consultas de descubrimiento incluyeron, entre otras, `"fuzzy-set qualitative comparative analysis" software`, `fsQCA calibration Likert survey software`, `fsQCA reliability validity workflow`, `QCA robustness software calibration anchors`, `fsQCA Shiny application` y `fsQCA reproducible report R package`. Se añadieron rastreos hacia atrás y hacia delante y comprobaciones en Crossref, OpenAlex, Google Scholar, CRAN y su archivo, CRAN Search, GitHub, COMPASSS y sitios oficiales.

El flujo calculado desde los archivos aprobados fue:

1. **Identificación:** 32 filas de búsqueda de herramientas o inspección, con 121 apariciones de herramientas revisadas en total.
...
| [SetMethods](https://cran.r-project.org/web/packages/SetMethods/index.html) | `no_verificado` | `no_verificado` | `parcial` | `parcial` | `si` | `si` | `no_verificado` | `parcial` | `si` | `parcial` |
...
La robustez aparece como `si` en [QCApro](https://cran.r-project.org/src/contrib/Archive/QCApro/QCApro_1.1-2.tar.gz), [ThSQCA](https://cran.r-project.org/web/packages/ThSQCA/index.html) y [QCAfalsePositive](https://doi.org/10.1093/pan/mpv017); es `parcial` en [SetMethods](https://cran.r-project.org/web/packages/SetMethods/index.html), [Kirq](https://grundrisse.org/qca/kirq) y [FSGoF](https://grundrisse.org/qca/eliason/), y `no_verificado` en las ocho restantes. La distribución sugiere especialización complementaria, no una innovación metodológica aún no conocida.

La clasificación parcial de SetMethods reconoce que el paquete implementa un protocolo publicado, pero dos rutas relevantes presentan defectos reproducibles. Los defectos están en estado **reportado**: se conserva una constancia local no sensible del reporte, pero no existe evidencia pública verificable de confirmación o corrección. Los ejemplos reproducibles locales muestran el fallo de `rob.cases` en [docs/referencias/setmethods-4.1-reprex.R](referencias/setmethods-4.1-reprex.R) y el resultado silenciosamente incompleto de `rob.fit` en [docs/referencias/robfit-solucion-media-ignorada.R](referencias/robfit-solucion-media-ignorada.R). No se identifica a la persona destinataria ni se publica correspondencia privada.
